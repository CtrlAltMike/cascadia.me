...
The county/regional-district baseline and the independent municipality and Indigenous-government audits are complete for the three requested regions. The municipal scopes contain 281 incorporated places in Washington, 241 in Oregon, and 161 in B.C. Every government has one dated outcome in `coverage-review.json`; all three remaining-independent-review counts are zero. Oregon's 25-item directory gap still means an incorporated city's normalized name was absent from the official OR-Alert search data on the review date—it does not mean ZIP routing or an alert service is unavailable—and each gap now also has an independent official-site outcome.

The Indigenous-government scopes contain 29 federally recognized Tribes in Washington, 9 in Oregon, and 204 First Nations in B.C.; all three remaining-independent-review counts are also zero. The Province reports 204 First Nations while CivicInfo displayed 208 directory records on the review date, so the ledger explicitly documents the four intergovernmental directory organizations excluded from the 204-government scope. The B.C.-wide First Nations’ Emergency Services Society record remains a support resource and is not counted as a Nation-specific alert authority. Signals contains no current-incident feed and no hand-drawn incident perimeters.

`coverage-audit.json` holds the aggregate baselines and manually confirmed named findings. `coverage-review.json` holds all 925 per-government outcomes: 14 dedicated records, 238 confirmed shared-system results, 368 official-site referrals, 183 reviews where no public source was confirmed, and 122 unavailable official sites. “No public source confirmed” is deliberately not a claim that no service exists. Automated official-site referrals are research leads only; they do not create a public listing, enrollment recommendation, or perimeter until manually confirmed. Both files, all six roster counts, every unique government, evidence URL, outcome total, source method, and referenced Signals resource are machine-validated. A municipality contained by a county or regional district is never assumed to use that government's alert system. A Tribe or First Nation is never assigned an alert authority from reservation, trust-land, treaty, ancestral-territory, or regional geometry.

## Validation and freshness

Run `node signals/scripts/validate-registry.mjs` to check record count, unique IDs, verb-led action types and URLs, provenance, review dates, coverage keys, optional metadata shapes, the aggregate and 925-entry coverage-review contracts, and the confirmed editorial review in `action-review.json`. The action-review file lists every current public record in exactly one evidence batch and seals its action URL, label, applicability note, and Start Here eligibility with a SHA-256 snapshot. Any later editorial change fails validation until the affected batch is reviewed and re-sealed.
